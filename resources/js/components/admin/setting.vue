<template>
    <div class="content ">
        <header-admin :user="user" :title="$t('site-settings')">
            <button v-show="user.role.settings.site" type="button" @click="changeSetting()" class="btn btn-sm font-bold btn-success w-20 ml-2">
                <div v-show="!load">{{ $t('edit') }}</div>
                <div v-show="load">
                    . . .
                </div>
            </button>
            {{ $t('last-update') + ": " + siteSetting.updateTime }}
        </header-admin>
        <div v-show="errorLoadState" class="alert alert-primary-soft show flex items-center mb-2" role="alert">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                 stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"
                 class="feather feather-alert-circle w-6 h-6 ml-2">
        ...
        <div v-show="!user.role.settings.site"
        ...
        <div v-show="user.role.settings.site && !errorLoadState">
            <form @submit.prevent="">
                ...
                <div class="col-span-12 sm:col-span-12">
                    <div class="input-group">
                        ...
                        <input @keyup.enter="" type="text" class="form-control"
                               :placeholder="$t('site-title' )"
                               v-model="siteSetting.siteTitleFa">
                    </div>
                </div>
                <div class="col-span-12 sm:col-span-12 mt-3">
                    <div class="input-group">
                        <input @keyup.enter="" type="text" dir="ltr"
                               ...
                               v-model="siteSetting.siteTitleEn">
                        <div id="input-group-email" class="input-group-text">
                ...
                <div class="col-span-6 sm:col-span-12 mt-3 d-flex">
                    <div class="col-span-4">
                        <label for="logo-light"> {{ $t('logo-light') }} </label>
                        <input id="logo-light" type="file" class="form-control"
                               ref="logo-light" @change="onChangeLogoLight">
                    </div>
                    <div class="col-span-4 mt-3">
                        <label for="logo-dark">{{ $t('logo-dark') }}</label>
                        <input id="logo-dark" type="file" class="form-control"
                               ref="logo-dark" @change="onChangeLogoDark">
                    </div>
                    <div class="col-span-4 mt-3">
                        <label for="fav-icon">{{ $t('fav-icon') }}</label>
                        <input id="fav-icon" type="file" class="form-control"
                               ref="fav-icon" @change="onChangeFavIcon">
                    </div>
                </div>
            ...
            <div class="col-span-12 mt-3 flex justify-between p-3 items-center">
                <div v-show="siteSetting.logoLight!=''" class="bg-gray-300 rounded p-2 ">
                    <img :src="siteSetting.logoLight" alt="">
                    <button @click="deleteLogo('light')"
                            class="text-blue-500 font-bold w-full mt-3 btn btn-outline-dark">
                        {{ $t('delete') + " " + $t('logo-light') }}
                    </button>
                </div>

                <div v-show="siteSetting.logoDark!=''" class="bg-gray-300 rounded p-2 ">
                    <img :src="siteSetting.logoDark" alt="">
                    <button @click="deleteLogo('dark')"
                            class="text-blue-500 font-bold w-full mt-3 btn btn-outline-dark">
                        {{ $t('delete') + " " + $t('logo-dark') }}
                    </button>
                </div>

                <div v-show="siteSetting.favIcon!=''" class="bg-gray-300 rounded p-2 ">
                    <img :src="siteSetting.favIcon" alt="">
                    <button @click="deleteLogo('fav')"
                            class="text-blue-500 font-bold w-full mt-3 btn btn-outline-dark">
                        {{ $t('delete') + " " + $t('fav-icon') }}
                    </button>
                </div>
            ...
            <button class="btn btn-sm btn-dark font-bold mb-2" @click="add">{{
                    $t('add') + " " + $t('fix-meno')
                }}
            </button>
            <draggable
...
export default {
    ...
    watch: {
        $route: {
            immediate: true,
            handler(to, from) {
                document.title = this.$t('site-settings') + ' | ' + this.$t('adminTitlePage');
            }
        },
    },
