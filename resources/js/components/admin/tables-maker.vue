<template>
    <div class="content ">
        ...
        <header-admin :user="user"
            ...
            <button v-show="user.role.table.add" type="button" @click="addHeader()"
                    class="btn btn-sm font-bold btn-success ml-2">
                {{ $t('add-header') }}
            </button>
            <button v-show="user.role.table.add" type="button" @click="addRow()"
                    class="btn btn-sm font-bold btn-success ml-2">
                {{ $t('add-data') }}
            </button>
            <button v-show="user.role.table.add" type="button" @click="addTable()"
                    class="btn btn-sm font-bold btn-dark ml-2">
                {{ $t('submit') }}
            </button>
        </header-admin>
        <div v-show="!user.role.table.add"
        ...
        <div v-show="user.role.table.add" class="box intro-y p-4">
            <div v-show="error"
                ...
                <table class="mt-2">
                    ...
                    <tr class="inline">
                        <th v-for="(item,index) in header">
                            <input type="text" v-model="item.title" class="border-2 p-2 rounded border-solid border-theme-15 font-bold">
    ...
    methods: {
        ...
        addTable() {
            if (this.table.title != '' && this.table.key != '') {
                ...
                axios.post("/table/add", sentData).then((response) => {
                    if (response.data == 'success') {
                        this.table.title = '';
                        this.table.language = 'all';
                        this.header = [];
                        this.data = [];
