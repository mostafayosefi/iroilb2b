<template>
    <div class="content ">
        <header-admin :user="user" :title="$t('add') + ' ' + $t('page') "></header-admin>

        <div v-show="!user.role.page.add"
        ...
        <div v-show="user.role.page.add" class="pos intro-y grid grid-cols-12 gap-5 mt-5">
            ...
            <div class="intro-y col-span-12 lg:col-span-3 text-left">
                <button class="btn mb-4 btn-lg border-0" :class="page.status=='publish'? 'btn-green' :'btn-dark' "
                    ...
                    <span v-show="page.status=='publish'">{{ $t('public') }}</span>
                </button>
...
export default {
    ...
    watch: {
        $route: {
            immediate: true,
            handler(to, from) {
                document.title = this.$t('add') + ' ' + this.$t('page') + ' | ' + this.$t('adminTitlePage');
            }
        },
    },
