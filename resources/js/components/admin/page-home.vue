<template>
    <div class="content ">
        <header-admin :user="user" :title="$t('edit') + ' ' + $t('page')  + ' ' + $t('home') ">
                    ...
                    <div class="post__content tab-content">
                        <div id="content" class="tab-pane p-5 active" role="tabpanel">
                                ...
                                <template #item="{ element, index }">
                                    <li class="bg-white mt-1 p-3 rounded">
                                        ...
                                        <label class="mx-2 font-bold">
                                            {{ $t('show-position') }}
                                            <select class="form-control mt-2" v-model="element.show">
                                                <option value="search">
                                                    {{ $t('before') + ' ' + $t('search') }}
                                                </option>
                                                ...
                                                <option value="company">
                                                    {{ $t('before') + ' ' + $t('companies') }}
                                                </option>
                                                <option value="blog">
                                                    {{ $t('before') + ' ' + $t('posts') }}
                                                </option>
                                                <option value="words">
                                                    {{ $t('before') + ' ' + $t('customers-words') }}
                                                </option>
                                            </select>
                                        </label>
                                        ...
                                        <label class="mx-2 font-bold">
                                            {{ $t('title') }}</label>
                                        <input type="text" class="form-control mb-2" v-model="element.title"/>
                                        <br>
                                        <label class="mx-2 font-bold">{{ $t('text') }}</label>
                                        <textarea v-model="element.text" class="form-control mb-2"> </textarea>
                                        ...
                                        <label class="mx-2 font-bold">
                                            {{ $t('color-background') }}
                                            <input type="color" v-model="element.background">
                                        </label>
                            ...
                            <span class="mx-2 px-2 rounded-full bg-theme-21 text-xs text-white"
                                  :class="home.seoDescription.fa.length<160 ? 'bg-theme-' : 'bg-theme-20'">{{
                                    home.seoDescription.fa.length
                                }}</span>
                            <textarea id="desc" class="form-control mt-3" rows="3"
                                      :placeholder="$t('descriptions') + ' ' + $t('seo')"
                                      v-model="home.seoDescription.fa" maxlength="170">
                            </textarea>
                        </div>
                        ...
                        <div id="meta-title" role="tabpanel" class="tab-pane p-5">
                            ...
                            <label for="desc">{{ $t('descriptions') + ' ' + $t('seo') }}</label>
                            <span class="mx-2 px-2 rounded-full bg-theme-21 text-xs text-white"
                                  :class="home.seoDescription.en.length<160 ? 'bg-theme-' : 'bg-theme-20'">{{
                                    home.seoDescription.en.length
                                }}</span>
                            <textarea id="desc" class="form-control mt-3" rows="3"
                                      :placeholder="$t('descriptions') + ' ' + $t('seo')"
                                      v-model="home.seoDescription.en" maxlength="170">
                            </textarea>
                        </div>
                    </div>
                </div>
...
export default {
    ...
    methods: {
        removeAt(idx, lang) {
            if (lang == 'fa') {
                this.home.fa.splice(idx, 1);
            }
            if (lang == 'en') {
                this.home.en.splice(idx, 1);
            }
        },
        add(lang) {
            if (lang == 'fa') {
        ...
        updatePage() {
            ...
            formData.append('id', this.home.id);
            formData.append('body_fa', JSON.stringify(this.home.fa));
            formData.append('body_en', JSON.stringify(this.home.en));
            formData.append('description_fa', this.home.seoDescription.fa);
            formData.append('description_en', this.home.seoDescription.en);
    ...
    created() {
        axios.get('/page/special/' + this.home.id,
            {
                params: {
                    api_token: this.$cookies.get('user').token
                }
            }).then(response => {
            this.home.id = response.data.data.id;
            this.home.fa = response.data.data.bodyFa ? response.data.data.bodyFa : [];
            this.home.en = response.data.data.bodyEn ? response.data.data.bodyEn : [];
            this.home.seoDescription.fa = response.data.data.seoDescriptionFa;
            this.home.seoDescription.en = response.data.data.seoDescriptionEn;
            this.$emit('loadingHide');
        }).catch((error) => {
            this.$emit('loadingHide');
        });
    ...
    watch: {
        $route: {
            immediate: true,
            handler(to, from) {
                document.title = this.$t('edit') + ' ' + this.$t('page') + ' ' + this.$t('home') + ' | ' + this.$t('adminTitlePage');
            }
        },
    },
